...
the DC).  Polarization is P_3 (axial component); the drive strain is proportional to sin(w t),
...
def static_Pz(lam, args):
    """Exact equilibrium (Maxwell fully relaxed) P_3 at a HELD stretch lam.
    Junction sits at the k-weighted centroid (independent of eq_frac); with
    matched stiffness (m=1) this is the geometric centroid and P_3 == 0."""
...
def P_star_of(args):
    ...
    All polarization data are reported as P_3/P*, which is O(1) and independent
...
def study_verify(outdir):
    """Verification: (i) the symmetric cell gives an exact null (reported in
    prose), (ii) the low-frequency loss recovers M1 ~ (d-1).  The drag sweep
    itself is the subject of fig_quadrupole and is not repeated here."""
    ...
    resid = np.max(np.abs(aout_lo - slope * x)) / max(aout_lo.max(), 1e-30)
    print(f"    [verify] symmetric null |P_3|/P* = {abs(met0['dc'])/Ps:.1e}; "
          f"(d-1) fit slope = {slope:.4e}, max rel. residual = {resid:.1e}")
    dump(outdir, "verify", ["d_minus_1", "a_out_over_Pstar"], [x, aout_lo])

    fig, ax = plt.subplots(figsize=(3.5, 3.1))
    ax.plot(x, aout_lo, "s", color=C_OUT, ms=5.5, label="simulation")
    ax.plot(x, slope * x, "--", color="#666", lw=1.2, label=r"$\propto(d-1)$")
    ax.set(xlabel=r"$d-1$",
           ylabel=rf"$|a_{{\rm out}}|/P_\star$  ($\omega\tau_{{\rm ref}}={wlo}$)")
...
def study_coarsen(outdir):
    ...
    ax[0].plot(ms, amp, "^--", color=C_OUT, lw=1.6, ms=4.5,
               label=r"dynamic  $|P_3^{(1)}|/P_\star$")
    ax[0].axhline(0, color="#bbb", lw=0.6)
    ax[0].set(xlabel=r"coarsening ratio $m$", ylabel=r"$P_3/P_\star$")
    ...
    ax[1].set(xlabel=r"$\lambda$", ylabel=r"$P_3/P_\star$")
...
def study_incompat(outdir):
    ...
    print(f"    [incompat] static P_3/P* varies by "
    ...
    ax[0].axhline(0, color="#bbb", lw=0.6)
    ax[0].set(xlabel=r"degree of cure $\eta$", ylabel=r"static $P_3/P_\star$",
    ...
    ax[1].set(xlabel=r"degree of cure $\eta$",
...
def study_quad(outdir):
    ...
    print(f"    [quad] max |static|/P* = {np.abs(dc).max():.2e}; "
          ...
          f"min AC/static ratio = {(ac[1:]/np.abs(dc[1:])).min():.0f}x")
    dump(outdir, "sluggish", ["d", "DC_over_Pstar", "a_in_over_Pstar",
                              "a_out_over_Pstar"], [ds, dc, ain, aout])
    ...
    ax.plot(ds, np.abs(dc), "o:", color=C_DC, lw=1.4, ms=4,
            label=r"static $|\langle P_3\rangle|$")
    ax.set(xlabel=r"drag ratio $d$", ylabel=r"$P_3/P_\star$")
