...
Features a dual-grid Omega generator for targeted high-resolution sweeps.
"""
...
def parse_arguments():
    ...
    # Dual-scale Omega parameters
    parser.add_argument('--omega_min_1', type=float, default=0.1,
                        help='Minimum Angular Frequency for Region 1 (Broad)')
    parser.add_argument('--omega_max_1', type=float, default=50.0,
                        help='Maximum Angular Frequency for Region 1 (Broad)')
    parser.add_argument('--n_omega_1', type=int, default=15,
                        help='Number of omega values in Region 1 (log scale)')
    
    parser.add_argument('--omega_min_2', type=float, default=5.0,
                        help='Minimum Angular Frequency for Region 2 (Fine Focus)')
    parser.add_argument('--omega_max_2', type=float, default=15.0,
                        help='Maximum Angular Frequency for Region 2 (Fine Focus)')
    parser.add_argument('--n_omega_2', type=int, default=15,
                        help='Number of omega values in Region 2 (log scale)')
...
def generate_parameter_grid(args):
    """Generate (dP, omega) parameter grid with smart dual-scale omega."""
    dP_vals = np.linspace(args.dP_min, args.dP_max, args.n_dP)
    
    # Generate the two raw omega regions
    omega_vals_1 = np.logspace(np.log10(args.omega_min_1), np.log10(args.omega_max_1), args.n_omega_1)
    omega_vals_2 = np.logspace(np.log10(args.omega_min_2), np.log10(args.omega_max_2), args.n_omega_2)
    
    # Define fine grid boundaries and spacings
    f_start = omega_vals_2[0]
    f_end = omega_vals_2[-1]
    
    # Calculate the spacing at the edges of the fine grid
    if len(omega_vals_2) > 1:
        delta_lower = omega_vals_2[1] - omega_vals_2[0]
        delta_upper = omega_vals_2[-1] - omega_vals_2[-2]
    else:
        delta_lower, delta_upper = 0.1, 0.1

    # 1. Filter and shift the coarse points
    smart_coarse_vals = []
    for c in omega_vals_1:
        # Check if the coarse point is inside the fine range
        if c >= f_start and c <= f_end:
            continue  # Skip this point entirely
            
        # 2. Check and shift points that are too close to the boundaries
        if c < f_start and (f_start - c) < delta_lower:
            c = max(1e-5, f_start - delta_lower)  # Shift left, prevent negative/zero frequency
        elif c > f_end and (c - f_end) < delta_upper:
            c = f_end + delta_upper  # Shift right
            
        smart_coarse_vals.append(c)
        
    # Concatenate, automatically sort low-to-high, and remove exact duplicates
    omega_vals = np.unique(np.concatenate((smart_coarse_vals, omega_vals_2)))
...
def main():
    ...
    print("="*60)
    print("SONICATION DUAL-GRID PARAMETER STUDY (dP vs omega)")
    print("="*60)
    ...
    print(f"  omega 1: [{args.omega_min_1}, {args.omega_max_1}] ({args.n_omega_1} points)")
    print(f"  omega 2: [{args.omega_min_2}, {args.omega_max_2}] ({args.n_omega_2} points)")
    print(f"Total unique simulations: {args.n_dP * len(omega_vals)}")
