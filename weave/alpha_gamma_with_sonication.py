#!/usr/bin/env python3
"""
Driver script for α-γ parameter study of weave potential with Sonication.
...
(if Kramers theory is correct). This script adds acoustic sonication 
to investigate if non-thermal energy injection shifts this optimal temperature.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Run α-γ parameter study for weave simulations with sonication',
    ...
    parser.add_argument('--dP', type=float, default=0.0,
                       help='Acoustic pressure amplitude')
    parser.add_argument('--omega', type=float, default=1.0,
                       help='Acoustic angular frequency')
    parser.add_argument('--wave_type', type=str, default='traveling',
                       choices=['traveling', 'standing'],
                       help='Type of acoustic wave')
    parser.add_argument('--theta', type=float, default=0.0,
                       help='Angle of acoustic wave')
    parser.add_argument('--wavelength', type=float, default=1.0,
                       help='Spatial wavelength of acoustic wave')
    ...
    parser.add_argument('--study_dir', type=str, default='alpha_gamma_sonication_study',
                       help='Base directory for study results')
    parser.add_argument('--weave_script', type=str, default='./weave_parallel_sonication.py',
                       help='Path to weave script with sonication enabled')
...
def generate_parameter_grid(args):
    """
    Generate (α, γ) parameter grid with fixed sonication settings.
    """
...
def run_simulation(params, outdir, weave_script, dry_run=False):
    ...
    # Optional: only print if debugging, it gets spammy with many cores
    # print(f"\nRunning: {cmd_str}")
...
def extract_mobility_grid(results, alpha_vals, gamma_vals, epsx, L, A):
...
def run_params(local_args):
    ...
    # Using flush=True ensures HPC systems actually print this live
    print(f"[{i+1}/{n}] α={alpha:.4f}, γ={gamma:.4f}", flush=True)
...
def main():
    ...
    print("="*60)
    print("α-γ PARAMETER STUDY WITH SONICATION")
    print("="*60)
    print(f"\nStudy directory: {args.study_dir}")
    print(f"\nFixed physics parameters:")
    print(f"  ε_x = {args.epsx:.2f} (x-forcing)")
    print(f"\nSonication Settings:")
    print(f"  dP = {args.dP}, omega = {args.omega}, wave = {args.wave_type}, theta = {args.theta}")
    print(f"\nParameter ranges:")
    ...
    mu_xx_grid, D_xx_grid = extract_mobility_grid(
        results, alpha_vals, gamma_vals, args.epsx, args.L, args.A)
