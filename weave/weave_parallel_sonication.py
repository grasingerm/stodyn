...
parser.add_argument('--dP', type=float, default=0.0, help='Amplitude of the sonication pressure wave')
parser.add_argument('--omega', type=float, default=0.0, help='Angular frequency of the sonication wave')
parser.add_argument('--wavelength', type=float, default=1.0, help='Spatial wavelength of the sonication wave')
parser.add_argument('--theta', type=float, default=0.0, help='Direction of the sonication wave in degrees')
parser.add_argument('--wave_type', type=str, default='traveling', help='Type of sonication wave: "traveling" or "standing"')
...
def F_sonication(x, y, t, dP, omega, kx, ky, wave_type):
    """
    Calculate the time-dependent force from the ultrasonic pressure wave.
    F = -∇P
    """
    # If amplitude is 0, skip the math and return a zero vector
    if dP == 0.0:
        return np.array([0.0, 0.0])
        
    if wave_type == 'traveling':
        phase = kx*x + ky*y - omega*t
        Fx = -dP * kx * np.cos(phase)
        Fy = -dP * ky * np.cos(phase)
        
    elif wave_type == 'standing':
        spatial_part = np.cos(kx*x + ky*y)
        temporal_part = np.cos(omega*t)
        Fx = -dP * kx * spatial_part * temporal_part
        Fy = -dP * ky * spatial_part * temporal_part
...
def run_single_trajectory_BAOAB(params):
    ...
    # Unpack sonication parameters (with safe defaults)
    dP = params.get('dP', 0.0)
    omega = params.get('omega', 0.0)
    kx = params.get('kx', 0.0)
    ky = params.get('ky', 0.0)
    wave_type = params.get('wave_type', 'traveling')
    ...
    for i in range(nsteps - 1):
        ...
        # B: Deterministic force, F(x, t_current)
        F = Fp - gradU(x[i], y[i], A, a, L, M) + F_sonication(x[i], y[i], t_current, dP, omega, kx, ky, wave_type)
        ...
        # B: second half velocity step, F(xnew, t_next)
        F = Fp - gradU(xvec_new[0], xvec_new[1], A, a, L, M) + F_sonication(xvec_new[0], xvec_new[1], t_next, dP, omega, kx, ky, wave_type)
...
if __name__ == "__main__":
    ...
    # Convert sonication wavelength and angle to wavevector components
    theta_rad = np.radians(args['theta'])
    k_mag = 2 * np.pi / args['wavelength']
    args['kx'] = k_mag * np.cos(theta_rad)
    args['ky'] = k_mag * np.sin(theta_rad)

    # Calculate Dimensionless Sonication Parameters
    if A != 0:
        args['eps_son'] = args['dP'] * L / A  # Dimensionless wave amplitude
    else:
        args['eps_son'] = np.nan

    args['Omega'] = args['omega'] / args['tau']  # Dimensionless frequency
